...
public:
    ...
    DSString(const std::string &); // constructor that converts a string
    ...
    std::string string() { return std::string(this->data); }
    ...
    friend std::istream &getline(std::istream & in, DSString & dsstr, char delim = '\n'){ // adds the inputted string to DSString
        // clean up old data
        delete[] dsstr.data;
        dsstr.len = 0;

        size_t tempPkgSize = 5; //  initialized to a small value to minimize memory usage initially
        //( we increase the array in groups of 5 chars
        char* newData = new char[tempPkgSize];

        char tempChar; // Holds each char from the input buffer

        // Read in each char from the input buffer until we hit a newline
        while (in.get(tempChar) && tempChar != delim)
        {
            dsstr.len++; // Holds the actual length of the data that's been read in

            // Scale if needed
            if (dsstr.len > tempPkgSize)
            {
                // Expand char array's package size (multipy by 2)
                tempPkgSize = tempPkgSize * 2;
                char *tempData = new char[tempPkgSize];
                ...
                // Set newData equal to tempData
                newData = tempData;
            }

            newData[dsstr.len - 1] = tempChar;
        }
    ...
    // friend std::vector<DSString> tokenizeTweet(const DSString &);
